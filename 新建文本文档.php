...
$url = isset($_GET['url']) ? $_GET['url'] : "https://wwn.lanzouh.com/iPhEb06dmysj";
...
if (strstr($softInfo, "手机Safari可在线安装") != false) {
      if(strstr($softInfo, "n_file_infos") != false){
        ...
        $lanzouId = $lanzouId[1];
        $ipaInfo = MloocCurlGet("https://www.lanzoui.com/tp/" . $lanzouId, 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_1 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.0 Mobile/14E304 Safari/602.1');
...
if(strstr($softInfo, "function down_p(){") != false){
    ...
    $softInfo = MloocCurlPost($post_data, "https://www.lanzoui.com/ajaxm.php", $url);
    $softName[1] = json_decode($softInfo,JSON_UNESCAPED_UNICODE)['inf'];
}else{
    preg_match("~\n<iframe.*?name=\"[\s\S]*?\"\ssrc=\"\/(.*?)\"~", $softInfo, $link);
    $ifurl = "https://www.lanzoui.com/" . $link[1];
    ...
    $softInfo = MloocCurlPost($post_data, "https://www.lanzoui.com/ajaxm.php", $ifurl);
}
